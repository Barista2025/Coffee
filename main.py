...
train_history=model.fit(x=x_Train_normalize,y=y_TrainOneHot,
            validation_split=0.2,epochs=2,batch_size=200,verbose=2)
show_train_history(train_history,'accuracy','val_accuracy')
show_train_history(train_history,'loss','val_loss')
scores=model.evaluate(x_Test_normalize,y_TestOneHot)
print()
ytest = y_TestOneHot.reshape(y_TestOneHot.shape[1],y_TestOneHot.shape[0])
print('accuracy',scores[1])
prediction=model.predict(x_Test)
print(str(prediction.shape))
print(str(ytest.shape))
print(prediction[10])
print(y_TestOneHot[10])
pd.crosstab(ytest,prediction,rownames=['label'],colnames=['prediction'])
